...
import { MY_API_KEY, baseUrl } from "../utilities/constant";
...
export const getGenres = createAsyncThunk("netflix/genres", async () => {
  const {
    data: { genres },
  } = await axios.get(`${baseUrl}/genre/movie/list?api_key=${MY_API_KEY}`);
  console.log(genres);
  return genres;
});

export const arrayOfMovieData = (array, moviesArray, genres) => {
  array?.forEach((movie) => {
    const moviesGenres = [];
...
export const fetchMovies = createAsyncThunk(
  ...
  async ({ type }, myThunk) => {
    ...
    return getMovieData(
      `${baseUrl}/trending/${type}/week?api_key=${MY_API_KEY}`,
      genres,
      true
    );
